...
[한국어](./releases.md) | [English](../en/releases.md) | [기술 설계 목차](../README.md)
...
프로그램, 스킬과 작업별 참조 문서, 에이전트, 설치·제거 도구와
...
설정도 같은 방식으로 나눈다. 개발용 [`.codex/config.toml`](../../../.codex/config.toml)은
배포하지 않으며, [`resources/codex.runtime.toml`](../../../resources/codex.runtime.toml)을
배포본의 `.codex/config.toml`로 넣는다. 개발 설정을 바꾸어도 사용자에게 전달되는
제품 설정은 바뀌지 않는다. 배포 검사에서 두 매핑과 스킬 참조 문서의 포함을 확인한다.
개인 설치의 저장·백그라운드 실행기가 사용하는 좁은 권한 프로필은 여전히
`personal_registration.py`가 별도로 생성하며, 이번 분리로 권한을 넓히지 않는다.
...
[`check_guides.py`](../../../scripts/check_guides.py)는 한글 README, 영문 사용자 안내,
웹의 설치 명령이 같은지, 다운로드 주소·설치 버전·웹 버전 표시가 `pyproject.toml`과
일치하는지 검사한다. 제품 배포 검사와 Pages 배포 모두 이 검사를 실행하며,
명령을 실행하거나 설치 파일을 내려받지는 않는다. 사용자에게 보이는 설명은
각 매체에 맞게 유지한다.
