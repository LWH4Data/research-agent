...
관련 확장: [첨부 PDF 저장과 설치 안내](./attachment-import.md) ·
[버전과 사용자용 Release 배포](./releases.md)
...
명령의 설정 스택에 불러오지 않는다. 저장소의 `.codex/config.toml`은 이 저장소를
직접 열 때의 프로젝트 설정이며, 설치된 실행기의 권한 프로필과 별개다.
...
[PDF 페이지와 시각 검토 근거](#pdf-페이지와-시각-검토-근거)를 따른다.
...
절차가 아니다. 저장 상태의 보장 범위는 [검토 상태의 의미](#검토-상태의-의미)를 따른다.
