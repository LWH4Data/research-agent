...
Markdown은 관련 내용과 페이지를 찾는 검색 인덱스입니다. 수식, 표, 그림, 정확한 수치가 필요한 질문에서는 Codex가 페이지 표시를 이용해 원본 PDF를 다시 확인하도록 `AGENTS.md`에 설정되어 있습니다. PDF 텍스트 추출 과정에서는 분수·첨자·행렬 배치와 복잡한 표의 열 정렬이 평면 텍스트로 바뀔 수 있으므로, Markdown만 근거로 정확한 기호나 셀 관계를 단정하지 않습니다.
