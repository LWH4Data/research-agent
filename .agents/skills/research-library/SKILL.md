...
for the mandatory source boundary, evidence labels, and model routing. Never
request write access to a registered source.

## Manage source locations

Use `./research-store source-list` to inspect registered locations. If the user
provides an exact folder or PDF path, register it with `./research-store
source-add <path>`. If no path is available, ask the user to run `bash
./add-source.sh` in a normal terminal so the native folder picker can open.

Removing a source uses `./research-store source-remove <id>`. Explain that this
disables future scans while retaining the original-path mapping. Its existing
Markdown snapshot remains searchable, and neither originals nor generated
Markdown are deleted. Adding the same path later re-enables it.

## Synchronize PDF documents

Delegate sync and state management to `library_manager`. It scans registered
...
failures, delegate those pages to `paper_converter`; do not ask the user to
...
If no source is registered, tell the user to run `bash ./add-source.sh`. If a
...
heading and `<!-- visual-review-pages: N -->` marker instead of the nearest
base page marker.
